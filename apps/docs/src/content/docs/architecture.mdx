...
- [Engine](./architecture/engine) — `ImageEngineAdapter` contract, `Record<string, any>` operation signature rule, Canvas implementation + `decodeResized`, streaming type contracts, Worker integration, stub engine pattern.
...
- **Engine version**: `canvasEngine.version = '0.1.0'`; stub engines have a `version` containing the `'stub'` marker, which `wrapAsImplementation()` in the Plugin layer detects and then sets `CapabilityImplementation.status = 'stub'`; `CapabilityRegistry.resolve()` skips them automatically
