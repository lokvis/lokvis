...
`@lokvis/cli` 是 Lokvis 的终端入口。它**不是**一个完整的 Node.js 运行时 —— Runtime 在设计上是浏览器优先的。CLI 适用于你在 CI 中通常需要的无副作用操作：能力发现、工作流结构校验、插件脚手架和版本检查。本指南展示如何以编程方式调用它，以及 Node.js 的能力边界在哪里。

## 1. 编程式 API

`@lokvis/cli` 导出 `runCLI(argv)`，与 `lokvis` bin 脚本使用的是同一个函数。你可以在任何 Node.js 脚本中导入它：
...
`runCLI` 直接写入 `process.stdout` 并返回 `Promise<void>`。遇到未知命令或无效参数时会抛出异常。

## 2. 能力查询：`runCLI(['capabilities'])`

`capabilities` 命令列出 `@lokvis/capability` 中 `BUILTIN_CAPABILITIES` 声明的每一项能力 —— 与 Runtime 用于初始化注册表的数组相同。
...
`@lokvis/schema` 导出 `validateWorkflow(data, options?)`，返回一个 Zod `SafeParseReturnType` 形状的结果。其形状为：
...
`validateWorkflow` 跨三层执行结构校验：

1. **Zod schema 形状** —— 必填字段、类型枚举（`assetTypeSchema`、`workflowCategorySchema`、`workflowOutputTypeSchema`）、节点/边数组。
2. **拓扑检查** —— 保留的哨兵 id（`__input__` / `__output__`）、节点 id 唯一性、边端点引用、自环、DAG 环检测（Kahn 算法）。
3. **能力兼容性**（可选，当传入 `resolveCapability` 时）—— 输入节点的 `inputTypes` 与 `workflow.inputs.type` 匹配；相邻节点的 `outputTypes ∩ inputTypes` 存在交集；输出节点的 `outputTypes` 与 `workflow.outputs.type` 匹配。schema 包不能从 `@lokvis/runtime` 导入，因此解析器以回调形式注入。
...
Lokvis Runtime 是**浏览器优先**的。图像引擎依赖于：

- `createImageBitmap` —— 用于将 blob 解码为位图
- `OffscreenCanvas` —— 用于脱离 DOM 的渲染
- `canvas.convertToBlob()` / `canvas.toBlob()` —— 用于编码

这些在 Node.js 中都不可用。CLI 在内部构造 Runtime 时会显式禁用 OPFS 和 IndexedDB：
...
因此 `lokvis run ./workflow.json ./input.png` 对于不触及浏览器 API 的能力（例如 `asset.rename`）可以正常工作，但图像 / 视频 / PDF 能力会抛出 `createImageBitmap is not supported in this environment` 或类似错误。

### 在 CI 中运行图像能力：Playwright

要在 CI 中运行图像工作流，请用 Playwright 驱动一个无头浏览器：
...
宿主页面必须以 COOP/COEP 头提供（参见 [嵌入 SDK](./embed-sdk#5-coopcoep-headers-for-sharedarraybuffer)），这样 `SharedArrayBuffer` 才可用。

## 5. CLI 命令

| 命令 | 签名 | 描述 |
|---|---|---|
| `run` | `lokvis run <workflow.json> [files...]` | 加载工作流 JSON，导入输入文件并执行。依赖浏览器的能力（图像、视频、PDF）在 Node 中会失败 —— 请改用 Playwright。 |
| `capabilities` | `lokvis capabilities`（别名：`caps`） | 列出 `@lokvis/capability` 中所有内置能力声明。 |
| `plugin create` | `lokvis plugin create <name> [target-dir]` | 用标准布局（`package.json`、`tsconfig.json`、`src/index.ts`、`src/plugin.ts`）脚手架生成一个新插件包。 |
| `mcp` | `lokvis mcp` | 启动 MCP 服务器（stdio JSON-RPC）。参见 [MCP 集成](../mcp)。 |
| `version` | `lokvis version`（别名：`--version`、`-v`） | 打印 CLI 版本。 |
| `help` | `lokvis help`（别名：`--help`、`-h`） | 打印帮助文本。 |

## 6. CI 示例

一个典型的 CI 步骤会校验工作流并打印能力：
...
使用 `tsx scripts/ci-check.ts` 运行，或编译后执行。它在无需启动浏览器的情况下即可成功完成。

## 下一步

- [CLI 参考](../cli) —— 快速命令概览。
- [MCP 集成](../mcp) —— 将同样的能力暴露给 AI 客户端。
- [Schema 参考](../architecture/runtime) —— `validateWorkflow` 背后的 Runtime 内部实现。
