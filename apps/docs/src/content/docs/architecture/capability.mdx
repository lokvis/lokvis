...
| `presets/image.ts` | `IMAGE_RESIZE` … `IMAGE_FILTER`, `IMAGE_CAPABILITIES` | 9 image capability presets |
| `presets/asset.ts` | `ASSET_RENAME`, `ASSET_ARCHIVE`, `ASSET_CAPABILITIES` | 2 cross-type asset capabilities |
| `presets/developer.ts` | `DEV_INSPECT_CAPABILITIES`, `DEV_INSPECT_ASSET`, `DEV_VALIDATE_WORKFLOW`, `DEV_PROFILE`, `DEVELOPER_CAPABILITIES` | 4 developer-tool capabilities |
| `presets/video.ts` | `VIDEO_COMPRESS` … `VIDEO_TO_GIF`, `VIDEO_CAPABILITIES` | 7 video capability presets (stub-only in Phase 1) |
| `presets/pdf.ts` | `PDF_MERGE` … `PDF_SIGN`, `PDF_CAPABILITIES` | 7 PDF capability presets (stub-only in Phase 1) |
