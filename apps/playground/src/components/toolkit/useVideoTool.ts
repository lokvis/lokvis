/**
 * useVideoTool — 单视频工具页共享 hook。
 *
 * 与 useImageTool / usePdfTool 对齐,但:
 * - 注入 videoToolsPlugin(浏览器版,全 stub,不加载 ffmpeg.wasm ~30MB)
 * - 输入为视频(用 inputUrl 直接 <video> 预览)
 * - 输出可能是视频 / 音频 / 图像(transcode → video,extract-audio → audio,
 *   to-gif / screenshot → image)
 *
 * 浏览器版视频处理为 stub:实际执行会抛 "not implemented in stub" 错误,
 * UI 层捕获并显示明确提示(引导用户使用 Node 端 mcp-server)。
 *
 * 抽出此 hook 后,Video 工具页只需定义参数面板 + buildWorkflow(params)。
 */
...
export interface UseVideoToolResult {
  ...
  /** 输入预览 URL(直接 <video src>) */
  inputUrl: string | null;
  /** 输入文件名 */
  inputName: string | null;
  /** 输入大小(bytes) */
  inputSize: number | null;
  /** 输入视频时长(秒,由 <video> metadata 读出) */
  ...
  /** 设置输入视频时长(<video> onLoadedMetadata 回调写入) */
...
export function useVideoTool(): UseVideoToolResult {
  ...
  // 初始化 runtime,注入 videoToolsPlugin(浏览器版全 stub,不加载 ffmpeg.wasm)
  useEffect(() => {
    let rt: LokvisRuntime | undefined;
    let cancelled = false;
    (async () => {
      try {
        rt = await createLokvis({ plugins: [videoToolsPlugin()] });
        if (cancelled) {
          void rt.cancel('all');
          return;
        }
    ...
    async (workflow: Workflow): Promise<void> => {
      ...
      try {
        ...
        if (result.status === 'completed' && result.outputs[0]) {
          ...
          const defaultName = `video-output-${Date.now()}.mp4`;
          setOutputBlob(blob);
