...
interface QuickPipelineContextValue {
  state: UseImagePipelineResult;
}

const QuickPipelineContext = createContext<QuickPipelineContextValue | null>(null);

function useQuickPipelineContext(): UseImagePipelineResult {
  const ctx = useContext(QuickPipelineContext);
  if (!ctx) {
    throw new Error(
      'QuickPipeline primitives 必须包裹在 <QuickPipeline.Root> 内使用,以获取 hook 状态。'
    );
  }
  return ctx.state;
}
...
export interface QuickPipelineRootProps extends UseQuickActionOptions<PipelinePreset> {
  children: ReactNode;
}

export function QuickPipelineRoot({ children, ...options }: QuickPipelineRootProps) {
  const state = useImagePipeline(options);
  return (
    <QuickPipelineContext.Provider value={{ state }}>
      {children}
    </QuickPipelineContext.Provider>
  );
}

// ─── Upload ─────────────────────────────────────────────────

export interface QuickPipelineUploadProps {
  className?: string;
  style?: CSSProperties;
...
export function QuickPipelineUpload({
  className,
  style,
  children,
  accept = 'image/*',
  ...
  const state = useQuickPipelineContext();
...
export interface QuickPipelinePresetSwitcherProps {
  className?: string;
  style?: CSSProperties;
...
export function QuickPipelinePresetSwitcher({
  className,
  style,
  renderButton,
...
export interface QuickPipelinePreviewProps {
  /** 渲染输入或最终输出图片 */
  type: 'input' | 'output';
  className?: string;
  style?: CSSProperties;
...
export function QuickPipelinePreview({
  type,
  className,
  style,
...
export interface QuickPipelineStepListProps {
  className?: string;
  style?: CSSProperties;
  /** 自定义渲染每个步骤(可选) */
  renderItem?: (step: {
 ...
 */
export function QuickPipelineStepList({
  className,
  style,
  renderItem,
  empty = null,
...
export interface QuickPipelineDownloadButtonProps {
  className?: string;
  style?: CSSProperties;
  fileName?: string;
  extension?: string;
...
export function QuickPipelineDownloadButton({
  className,
  style,
...
export interface QuickPipelineErrorDisplayProps {
  className?: string;
  style?: CSSProperties;
...
export function QuickPipelineErrorDisplay({
  className,
  style,
  format,
  empty = null,
...
export interface QuickPipelineResetButtonProps {
  className?: string;
  style?: CSSProperties;
  children?: ReactNode;
...
export function QuickPipelineResetButton({
  className,
  style,
  children = 'Reset',
...
export const QuickPipeline = {
  Root: QuickPipelineRoot,
  Upload: QuickPipelineUpload,
  PresetSwitcher: QuickPipelinePresetSwitcher,
  Preview: QuickPipelinePreview,
  StepList: QuickPipelineStepList,
  DownloadButton: QuickPipelineDownloadButton,
  ErrorDisplay: QuickPipelineErrorDisplay,
  ResetButton: QuickPipelineResetButton,
};
