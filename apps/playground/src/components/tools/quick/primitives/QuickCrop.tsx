...
interface QuickCropContextValue {
  state: UseQuickCropResult;
}

const QuickCropContext = createContext<QuickCropContextValue | null>(null);

function useQuickCropContext(): UseQuickCropResult {
  const ctx = useContext(QuickCropContext);
  if (!ctx) {
    throw new Error(
      'QuickCrop primitives 必须包裹在 <QuickCrop.Root> 内使用,以获取 hook 状态。'
    );
  }
  return ctx.state;
}
...
export interface QuickCropRootProps extends UseQuickActionOptions<CropPreset> {
  children: ReactNode;
}

export function QuickCropRoot({ children, ...options }: QuickCropRootProps) {
  const state = useQuickCrop(options);
  return (
    <QuickCropContext.Provider value={{ state }}>
      {children}
    </QuickCropContext.Provider>
  );
}

// ─── Upload ─────────────────────────────────────────────────

export interface QuickCropUploadProps {
  className?: string;
  style?: CSSProperties;
...
export function QuickCropUpload({
  className,
  style,
  children,
  accept = 'image/*',
  ...
  const state = useQuickCropContext();
...
export interface QuickCropPresetSwitcherProps {
  className?: string;
  style?: CSSProperties;
...
export function QuickCropPresetSwitcher({
  className,
  style,
  renderButton,
...
export interface QuickCropPreviewProps {
  type: 'input' | 'output';
  className?: string;
  style?: CSSProperties;
...
export function QuickCropPreview({
  type,
  className,
  style,
...
export interface QuickCropAreaProps {
  className?: string;
  style?: CSSProperties;
  /** 占位文案(无输入图片时显示) */
  placeholder?: ReactNode;
...
export interface QuickCropDownloadButtonProps {
  className?: string;
  style?: CSSProperties;
  fileName?: string;
  extension?: string;
...
export function QuickCropDownloadButton({
  className,
  style,
  fileName = 'cropped',
  extension,
...
export interface QuickCropErrorDisplayProps {
  className?: string;
  style?: CSSProperties;
...
export function QuickCropErrorDisplay({
  className,
  style,
  format,
  empty = null,
...
export interface QuickCropResetButtonProps {
  className?: string;
  style?: CSSProperties;
  children?: ReactNode;
...
export function QuickCropResetButton({
  className,
  style,
  children = 'Reset',
...
export const QuickCrop = {
  Root: QuickCropRoot,
  Upload: QuickCropUpload,
  PresetSwitcher: QuickCropPresetSwitcher,
  Preview: QuickCropPreview,
  CropArea: QuickCropArea,
  DownloadButton: QuickCropDownloadButton,
  ErrorDisplay: QuickCropErrorDisplay,
  ResetButton: QuickCropResetButton,
};
