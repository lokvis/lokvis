/**
 * ImageQuickResize 默认 UI(Layer 2)单测。
 *
 * 测试覆盖:
...
import ImageQuickResize from '../ImageQuickResize';
...
describe('ImageQuickResize 默认 UI', () => {
  beforeEach(() => {
    resetMocks();
    setMockState();
  });
  ...
  it('渲染根元素 .lokvis-quick-resize class', () => {
    const { container } = render(<ImageQuickResize />);
    const root = container.querySelector('.lokvis-quick-resize');
    expect(root).not.toBeNull();
  });

  it('渲染标题与副标', () => {
    render(<ImageQuickResize />);
  ...
  it('showDownloadButton=false 时,即使有输出也不显示下载按钮', () => {
    ...
    render(<ImageQuickResize showDownloadButton={false} />);
    expect(screen.queryByText('quickResize.download')).toBeNull();
  });

  it('showResetButton=false 时隐藏重置按钮', () => {
    render(<ImageQuickResize showResetButton={false} />);
  ...
  it('showDimension=false 时,即使有输出也不显示尺寸徽章', () => {
    ...
    render(<ImageQuickResize showDimension={false} />);
    expect(screen.queryByRole('status')).toBeNull();
  });

  it('theme prop 转 CSS 变量,应用到根元素', () => {
    const { container } = render(
      <ImageQuickResize theme={{ primary: '#00ff00', radius: '0' }} />
    );
  ...
  it('外层 className 追加到根元素', () => {
    const { container } = render(<ImageQuickResize className="my-custom-class" />);
    const root = container.querySelector('.lokvis-quick-resize');
  ...
  it('外层 style 合并到根元素', () => {
    const { container } = render(
      <ImageQuickResize style={{ margin: '20px' }} />
    );
    const root = container.querySelector('.lokvis-quick-resize') as HTMLElement;
  ...
  it('components prop 替换 UploadBox', () => {
    ...
    render(<ImageQuickResize components={{ UploadBox: CustomUpload }} />);
  ...
  it('components prop 替换 DownloadButton', () => {
    ...
    render(<ImageQuickResize components={{ DownloadButton: CustomDownload }} />);
