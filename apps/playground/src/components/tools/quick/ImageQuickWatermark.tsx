/**
 * ImageQuickWatermark — 一键加水印默认 UI(Layer 2)。
 *
 ...
 *
 * 默认布局与 ImageQuickCompress 类似,但 footer 中包含 TextInput(输入水印文字)。
 *
...
export interface QuickWatermarkComponents {
...
function presetLabel(preset: WatermarkPreset): string {
  return WATERMARK_PRESETS[preset].label;
}
...
// ─── ImageQuickWatermark Props ─────────────────────────────

export interface ImageQuickWatermarkProps extends UseQuickActionOptions<WatermarkPreset> {
  className?: string;
  style?: CSSProperties;
  /** 初始水印文字(默认 'Lokvis') */
  initialText?: string;
  ...
  theme?: QuickTheme;
  components?: Partial<QuickWatermarkComponents>;
}

// ─── 默认 UI 实现 ─────────────────────────────────────────

function ImageQuickWatermarkDefault({
  className = '',
  style,
  ...
  return (
    ...
    >
      <QuickWatermark.Root {...hookOptions} initialText={initialText}>
...
export default function ImageQuickWatermark(props: ImageQuickWatermarkProps) {
  return (
    <ErrorBoundary>
      <ImageQuickWatermarkDefault {...props} />
    </ErrorBoundary>
  );
}
