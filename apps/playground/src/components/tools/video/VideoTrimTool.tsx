 ...
 * Video Trim 工具页
 *
 * 导入视频 → 选择起止时间(start/end) → 调用 video.trim
 *
 * 浏览器版 plugin-video 为 stub,执行时会抛 "not implemented in stub" 错误。
 * trim 的 end 受限于 inputDuration(<video> metadata 读取),inputDuration
 * 为 null 时禁用 trim 按钮(需先加载视频)。
 */
...
export default function VideoTrimTool() {
  return (
    <ErrorBoundary>
      <VideoTrimToolContent />
    </ErrorBoundary>
  );
}

function VideoTrimToolContent() {
  const lang = useLang();
  ...
  const handleTrim = useCallback(async () => {
    ...
    const wf = buildSingleStepVideoWorkflow(
      ...
      'VideoTrim',
      'video',
      `Trim video from ${start}s to ${effectiveEnd}s`
    );
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-zinc-800 px-4 py-3">
        <h1 className="text-sm font-semibold text-zinc-100">{t('videoTrim.title')}</h1>
        <p className="mt-0.5 text-xs text-zinc-500">{t('videoTrim.subtitle')}</p>
      </div>
      ...
      <div className="flex flex-1 flex-col gap-4 overflow-auto p-4">
        <div className="grid grid-cols-1 gap-3 rounded-lg border border-zinc-800 bg-zinc-900/50 p-3 sm:grid-cols-12">
          ...
          <div className="sm:col-span-12">
            <p className="text-[10px] text-zinc-600">
              {maxDuration > 0
                ? `${t('videoTrim.durationLabel')} ${maxDuration.toFixed(1)}s · ${t('videoTrim.clipLength')} ${(effectiveEnd - start).toFixed(1)}s`
                : t('videoTrim.loadFirst')}
            </p>
          </div>
        </div>
        ...
        <div className="flex justify-center">
          ...
          >
            {tool.busy ? t('videoTrim.busy') : t('videoTrim.btn')}
          </button>
        </div>
        ...
        <VideoToolResultPanel
          ...
          uploadHint={t('videoTrim.uploadHint')}
          reselectLabel={t('videoTrim.reselect')}
