...
describe('engine-image-node operations', () => {
  ...
  describe('resize', () => {
    ...
    it('width + height + fit=cover (engine-image 不裁剪行为)', async () => {
      const { resize } = await import('../operations/transform.js');
      const out = await resize(testPng, {
        width: 100,
        height: 100,
      ...
      const dims = await getDimensions(out);
      // engine-image computeTargetSize 对 200x100 → cover 100x100:
      // targetRatio(1) < srcRatio(2),走 else 分支 → 200x100(不裁剪,
      // 与 sharp fit=cover 会裁剪的行为不同,本引擎与 engine-image 对齐)
  ...
  describe('crop', () => {
    it('应从 (x,y) 提取指定区域', async () => {
      const { crop } = await import('../operations/transform.js');
  ...
  describe('compress', () => {
    it('应输出 webp(默认格式)', async () => {
      const { compress } = await import('../operations/encode.js');
  ...
  describe('convert', () => {
    it('应把 png 转为 webp', async () => {
      const { convert } = await import('../operations/encode.js');
  ...
  describe('watermark', () => {
    it('文字水印应成功 composite(产出可被 sharp 解码)', async () => {
      const { watermark } = await import('../operations/watermark.js');
  ...
  describe('utils', () => {
    it('computeTargetSize 与 engine-image 一致', async () => {
      const { computeTargetSize } = await import('../operations/utils.js');
    ...
    it('inferFormat 与 engine-image 一致', async () => {
      const { inferFormat } = await import('../operations/utils.js');
    ...
    it('normalizeQuality 边界', async () => {
      const { normalizeQuality } = await import('../operations/utils.js');
    ...
    it('throwIfAborted 与 engine-image 一致', async () => {
      const { throwIfAborted } = await import('../operations/utils.js');
    ...
    it('isSafeImageUrl 与 engine-image 一致', async () => {
      const { isSafeImageUrl } = await import('../operations/utils.js');
