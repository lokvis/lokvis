...
import { createAuthenticator, createBilling, createAiClient } from '@lokvis/cloud-bridge';
...
import { getAiToolRegistrations } from './tools/ai.js';
...
/**
 * 能力域(决定注册哪些 tools)。
 * 'image'(5 个 tool:resize/compress/convert/crop/watermark)、
 * 'pdf'(5 个 tool:merge/compress/split/rotate/watermark)、
 * 'video'(7 个 tool:compress/transcode/trim/merge/extract-audio/to-gif/screenshot)、
 * 'audio'(4 个 tool:trim/normalize/merge/transcode)均已实装,
 * 经 runtime.run 走完整 capability 系统(Node 端 ffmpeg-static / pdf-lib / sharp);
 * 'ai' 为占位(待 cloud-proxy caller 注入后补 tool handler)。
 */
...
export async function createLokvisMcpServer(
  ...
  if (domains.includes('video')) {
    await runtime.installPlugin(await videoToolsPluginNode());
  }
  if (domains.includes('audio')) {
    await runtime.installPlugin(await audioToolsPluginNode());
  }
  if (domains.includes('ai')) {
    // AI 能力需要 cloudCaller 注入才能实装;无 cloud 配置时安装无 caller 版本(stub)
    const cloudCaller = cloud ? createAiClient(cloud) : undefined;
    await runtime.installPlugin(aiToolsPlugin({ cloudCaller }));
  }
  ...
  // 收集 tool registration:所有 domain handler 均接收 runtime(走 capability 系统,
  ...
  const audioRegistrations = domains.includes('audio')
    ...
    : [];
  const aiRegistrations = domains.includes('ai')
    ? getAiToolRegistrations(runtime)
    : [];
