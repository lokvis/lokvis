/**
 * i18n 工具函数(@lokvis/embed-pdf 内部)。
 *
 * 所有翻译函数支持 translations overrides(来自 EmbedPdfI18nProvider)。
...
import { ui } from './ui';
import { usePdfI18nContext, type EmbedPdfTranslations } from './EmbedPdfI18nProvider';

// re-export 类型,方便外部 import { type Language } from '@lokvis/embed-pdf'
export type { Language };
// re-export EmbedPdfTranslations 供消费方构造 overrides 类型
export type { EmbedPdfTranslations };
 ...
 *
 * @param lang 当前语言
 * @param key 翻译 key(如 'pdfCompress.title')
 * @param overrides 消费方注入的翻译覆盖(可选)
 */
export function t(
  lang: Language,
  key: string,
  overrides?: EmbedPdfTranslations
): string {
 ...
 * 创建翻译函数(React 组件用)。
 *
 * 优先级:explicitOverrides 参数 → EmbedPdfI18nProvider.translations → 包内字典。
 *
 * @param lang 当前语言(由 useLang 推导)
 * @param explicitOverrides 组件 translations prop 传入的显式覆盖(优先级最高)
 */
export function useTranslations(lang: Language, explicitOverrides?: EmbedPdfTranslations) {
