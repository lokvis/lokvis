  ...
  return (
    ...
    >
      <button
        ...
        disabled={disabled}
        onClick={toggle}
        className={`relative inline-flex flex-shrink-0 items-center rounded-full transition-colors duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-1 ${s.track} ${
          isOn ? 'bg-indigo-600' : 'bg-zinc-200 dark:bg-zinc-700'
        }`}
        style={isOn ? { backgroundColor: 'var(--lokvis-primary, #6366f1)' } : undefined}
      >
        <span
          className={`inline-block transform rounded-full bg-white shadow-sm transition-transform duration-150 ${s.thumb} ${
