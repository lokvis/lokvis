...
import { ImageWatermark } from '../primitives/ImageWatermark';
...
describe('ImageWatermark 原语', () => {
  ...
  describe('Upload', () => {
    it('不在 Root 内使用时抛错', () => {
      ...
      expect(() => render(<ImageWatermark.Upload />)).toThrow(
        /必须包裹在 <ImageWatermark.Root> 内/
      );
      spy.mockRestore();
    });
    ...
    it('渲染 role=button + aria-label', () => {
      render(
        <ImageWatermark.Root>
          <ImageWatermark.Upload aria-label="Pick image" />
        </ImageWatermark.Root>
      );
    ...
    it('函数 children 接收 isDragging 状态', () => {
      render(
        <ImageWatermark.Root>
          <ImageWatermark.Upload>{({ isDragging }) => <span>{isDragging ? 'dragging' : 'idle'}</span>}</ImageWatermark.Upload>
        </ImageWatermark.Root>
      );
  ...
  describe('PresetSwitcher', () => {
    ...
    it('当前 preset 对应的 radio aria-checked=true', () => {
      render(
        <ImageWatermark.Root initialPreset="tile">
          <ImageWatermark.PresetSwitcher />
        </ImageWatermark.Root>
      );
      const radios = screen.getAllByRole('radio');
  ...
  describe('Preview', () => {
    it('无图片时渲染 placeholder', () => {
      render(
        <ImageWatermark.Root>
          <ImageWatermark.Preview type="input" placeholder="empty" />
        </ImageWatermark.Root>
      );
    ...
    it('有图片时渲染 <img>', () => {
      ...
      render(
        <ImageWatermark.Root>
          <ImageWatermark.Preview type="input" />
        </ImageWatermark.Root>
      );
  ...
  describe('TextInput', () => {
    ...
    it('placeholder 可自定义', () => {
      render(
        <ImageWatermark.Root>
          <ImageWatermark.TextInput placeholder="输入水印文字" />
        </ImageWatermark.Root>
      );
    ...
    it('initialText 反映在 input value 上', () => {
      render(
        <ImageWatermark.Root initialText="CustomMark">
          <ImageWatermark.TextInput />
        </ImageWatermark.Root>
      );
    ...
    it('maxLength 可自定义(默认 50)', () => {
      render(
        <ImageWatermark.Root>
          <ImageWatermark.TextInput maxLength={20} />
        </ImageWatermark.Root>
      );
      const input = screen.getByRole('textbox') as HTMLInputElement;
  ...
  describe('DownloadButton', () => {
    ...
    it('有输出时渲染 button', () => {
      ...
      render(
        <ImageWatermark.Root>
          <ImageWatermark.DownloadButton>下载</ImageWatermark.DownloadButton>
        </ImageWatermark.Root>
      );
  ...
  describe('ErrorDisplay', () => {
    ...
    it('自定义 format 函数', () => {
      ...
      render(
        <ImageWatermark.Root>
          <ImageWatermark.ErrorDisplay format={(e) => `Error: ${e}`} />
        </ImageWatermark.Root>
      );
  ...
  describe('ResetButton', () => {
    ...
    it('disabledWhenEmpty=false 时始终启用', () => {
      render(
        <ImageWatermark.Root>
          <ImageWatermark.ResetButton disabledWhenEmpty={false} />
        </ImageWatermark.Root>
      );
      expect(screen.getByRole('button')).toBeEnabled();
    });
