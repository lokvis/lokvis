/**
 * useImageCrop hook(Layer 0)单测。
 *
 * 测试覆盖:
 *   - 初始状态(默认 / 自定义 initialPreset)
 *   - IMAGE_CROP_PRESETS 常量正确性(aspectRatio / label)
...
describe('useImageCrop', () => {
  ...
  describe('初始状态', () => {
    ...
    it('initialPreset 可自定义', () => {
      const { result } = renderHook(() =>
        useImageCrop({ initialPreset: '16:9' })
      );
  ...
  describe('IMAGE_CROP_PRESETS 常量', () => {
    it('square aspectRatio=1', () => {
      expect(IMAGE_CROP_PRESETS.square.aspectRatio).toBe(1);
    });

    it('4:3 aspectRatio=4/3', () => {
      expect(IMAGE_CROP_PRESETS['4:3'].aspectRatio).toBeCloseTo(4 / 3, 5);
    });

    it('16:9 aspectRatio=16/9', () => {
      expect(IMAGE_CROP_PRESETS['16:9'].aspectRatio).toBeCloseTo(16 / 9, 5);
    });

    it('free aspectRatio=null', () => {
      expect(IMAGE_CROP_PRESETS.free.aspectRatio).toBeNull();
    });
  });
  ...
  describe('autoRun', () => {
    it('inputId 变化时触发 runWorkflow,使用 image.crop capability', async () => {
      ...
      const { rerender } = renderHook(() => useImageCrop());
      rerender();
      await act(async () => {
    ...
    it('free 预设传整图(无裁剪)', async () => {
      ...
      const { rerender } = renderHook(() =>
        useImageCrop({ initialPreset: 'free' })
      );
      rerender();
      await act(async () => {
        await Promise.resolve();
  ...
  describe('setPreset', () => {
    ...
    it('busy 时不重跑(但仍更新 preset)', async () => {
      const { result, rerender } = renderHook(() => useImageCrop());
      // 先触发 autoRun
      setMockState({
        inputId: 'input-1',
        ready: true,
  ...
  describe('onComplete', () => {
    it('outputBlob 出现时触发一次 onComplete', async () => {
      ...
      const { rerender } = renderHook(() => useImageCrop({ onComplete }));
      setMockState({
        inputId: 'input-1',
        ready: true,
        inputInfo: { width: 800, height: 600, size: 1000, format: 'PNG' },
