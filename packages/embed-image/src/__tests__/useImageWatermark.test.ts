/**
 * useImageWatermark hook(Layer 0)单测。
 *
 * 测试覆盖:
 *   - 初始状态(默认 / 自定义 initialPreset / 自定义 initialText)
 *   - IMAGE_WATERMARK_PRESETS 常量正确性
...
describe('useImageWatermark', () => {
  ...
  describe('初始状态', () => {
    it('默认 preset 为 small-br', () => {
      const { result } = renderHook(() => useImageWatermark());
    ...
    it('initialPreset 可自定义', () => {
      const { result } = renderHook(() =>
        useImageWatermark({ initialPreset: 'tile' })
      );
    ...
    it('initialText 可自定义', () => {
      const { result } = renderHook(() =>
        useImageWatermark({ initialText: 'Hello' })
      );
  ...
  describe('IMAGE_WATERMARK_PRESETS 常量', () => {
    it('small-br 为 bottom-right + fontSize=16 + opacity=0.7', () => {
      expect(IMAGE_WATERMARK_PRESETS['small-br'].position).toBe('bottom-right');
      expect(IMAGE_WATERMARK_PRESETS['small-br'].fontSize).toBe(16);
      expect(IMAGE_WATERMARK_PRESETS['small-br'].opacity).toBe(0.7);
    });

    it('large-center 为 center + fontSize=64 + opacity=0.5', () => {
      expect(IMAGE_WATERMARK_PRESETS['large-center'].position).toBe('center');
      expect(IMAGE_WATERMARK_PRESETS['large-center'].fontSize).toBe(64);
      expect(IMAGE_WATERMARK_PRESETS['large-center'].opacity).toBe(0.5);
    });

    it('tile 为 tile + fontSize=24 + opacity=0.3', () => {
      expect(IMAGE_WATERMARK_PRESETS['tile'].position).toBe('tile');
      expect(IMAGE_WATERMARK_PRESETS['tile'].fontSize).toBe(24);
      expect(IMAGE_WATERMARK_PRESETS['tile'].opacity).toBe(0.3);
    });
  });
  ...
  describe('autoRun', () => {
    it('inputId 变化时触发 runWorkflow,使用 image.watermark capability', async () => {
      const { rerender } = renderHook(() => useImageWatermark());
    ...
    it('自定义 initialText 作为 text 参数传入', async () => {
      const { rerender } = renderHook(() =>
        useImageWatermark({ initialText: 'Custom' })
      );
      setMockState({ inputId: 'input-1', ready: true });
      rerender();
      await act(async () => {
    ...
    it('autoRun=false 时不自动触发', async () => {
      const { rerender } = renderHook(() =>
        useImageWatermark({ autoRun: false })
      );
      setMockState({ inputId: 'input-1', ready: true });
      rerender();
      await act(async () => {
  ...
  describe('setPreset', () => {
    ...
    it('busy 时不重跑(但仍更新 preset)', async () => {
      const { result, rerender } = renderHook(() => useImageWatermark());
      // 先触发 autoRun
      setMockState({ inputId: 'input-1', ready: true });
      rerender();
      await act(async () => {
  ...
  describe('run()', () => {
    ...
    it('run() 使用当前 text + preset', async () => {
      const { result } = renderHook(() =>
        useImageWatermark({ initialPreset: 'tile', initialText: 'MyMark' })
      );
      await act(async () => {
        await result.current.run();
      });
