/**
 * EmbedImageResize 默认 UI(Layer 2)单测。
 *
 * 测试覆盖:
 *   - 默认渲染:标题 + 副标 + UploadBox + PresetSwitcher + Preview × 2 + footer
 *   - theme prop 转 CSS 变量
 *   - components prop 替换子组件
 *   - showPresetSwitcher / showBeforeAfter / showDimension / showDownloadButton / showResetButton
...
import EmbedImageResize from '../EmbedImageResize';
...
describe('EmbedImageResize 默认 UI', () => {
  beforeEach(() => {
    resetMocks();
    setMockState();
  });
  ...
  it('渲染根元素 .lokvis-quick-resize class', () => {
    const { container } = render(<EmbedImageResize />);
    const root = container.querySelector('.lokvis-quick-resize');
    expect(root).not.toBeNull();
  });

  it('渲染标题与副标', () => {
    render(<EmbedImageResize />);
    expect(screen.getByText('quickResize.title')).toBeInTheDocument();
    expect(screen.getByText('quickResize.subtitle')).toBeInTheDocument();
  });
  ...
  it('showPresetSwitcher=false 隐藏预设切换器', () => {
    render(<EmbedImageResize showPresetSwitcher={false} />);
  ...
  it('showDownloadButton=false 时,即使有输出也不显示下载按钮', () => {
    ...
    render(<EmbedImageResize showDownloadButton={false} />);
    expect(screen.queryByText('quickResize.download')).toBeNull();
  });

  it('showResetButton=false 时隐藏重置按钮', () => {
    render(<EmbedImageResize showResetButton={false} />);
    expect(screen.queryByText('quickResize.retry')).toBeNull();
  });

  it('showDimension=false 时,即使有输出也不显示尺寸徽章', () => {
    setMockState({
      inputInfo: { width: 1920, height: 1080, size: 1000, format: 'PNG' },
      outputInfo: { width: 1280, height: 720, size: 500, format: 'PNG' },
    });
    render(<EmbedImageResize showDimension={false} />);
    expect(screen.queryByRole('status')).toBeNull();
  });

  it('theme prop 转 CSS 变量,应用到根元素', () => {
    const { container } = render(
      <EmbedImageResize theme={{ primary: '#00ff00', radius: '0' }} />
    );
    const root = container.querySelector('.lokvis-quick-resize') as HTMLElement;
  ...
  it('外层 className 追加到根元素', () => {
    const { container } = render(<EmbedImageResize className="my-custom-class" />);
    const root = container.querySelector('.lokvis-quick-resize');
  ...
  it('外层 style 合并到根元素', () => {
    const { container } = render(
      <EmbedImageResize style={{ margin: '20px' }} />
    );
    const root = container.querySelector('.lokvis-quick-resize') as HTMLElement;
  ...
  it('components prop 替换 UploadBox', () => {
    ...
    render(<EmbedImageResize components={{ UploadBox: CustomUpload }} />);
  ...
  it('components prop 替换 DownloadButton', () => {
    ...
    render(<EmbedImageResize components={{ DownloadButton: CustomDownload }} />);
  ...
  it('F1: mode="dark" 时根元素 data-quick-mode="dark"', () => {
    const { container } = render(<EmbedImageResize mode="dark" />);
    const root = container.querySelector('.lokvis-quick-resize') as HTMLElement;
    expect(root.getAttribute('data-quick-mode')).toBe('dark');
  });

  it('F1: mode="light" 时根元素 data-quick-mode="light"', () => {
    const { container } = render(<EmbedImageResize mode="light" />);
    const root = container.querySelector('.lokvis-quick-resize') as HTMLElement;
  ...
  it('F2: busy=true 时渲染 processing 文案 + aria-busy', () => {
    ...
    expect(screen.getByText('quickResize.processing')).toBeInTheDocument();
    // aria-busy
  ...
  it('F2: busy=false 时不渲染 processing 文案', () => {
    ...
    render(<EmbedImageResize />);
    expect(screen.queryByText('quickResize.processing')).toBeNull();
  });
  ...
  it('F4: UploadBox 的 aria-label 使用 i18n dropHint 键', () => {
    render(<EmbedImageResize />);
    // t mock 返回 key 字符串,所以 aria-label = 'quickResize.dropHint'
    const uploadBtn = screen.getByRole('button', { name: 'quickResize.dropHint' });
