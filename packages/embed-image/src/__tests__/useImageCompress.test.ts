/**
 * useImageCompress hook 单测(Layer 0)。
 *
 * 测试覆盖:
 ...
 *
 * useImageTool 被整体 mock,只验证 useImageCompress 自身逻辑。
 */
...
import { useImageCompress, IMAGE_COMPRESS_PRESETS } from '../hooks/useImageCompress';
...
describe('useImageCompress', () => {
  ...
  it('initialPreset 可自定义', () => {
    const { result } = renderHook(() =>
      useImageCompress({ initialPreset: 'highQuality' })
    );
    expect(result.current.preset).toBe('highQuality');
  });

  it('IMAGE_COMPRESS_PRESETS 包含 3 个预设,所有格式都是 webp', () => {
    expect(Object.keys(IMAGE_COMPRESS_PRESETS).sort()).toEqual(['balanced', 'highQuality', 'small']);
    expect(IMAGE_COMPRESS_PRESETS.balanced.quality).toBe(80);
    expect(IMAGE_COMPRESS_PRESETS.highQuality.quality).toBe(92);
    expect(IMAGE_COMPRESS_PRESETS.small.quality).toBe(65);
    Object.values(IMAGE_COMPRESS_PRESETS).forEach((v) => expect(v.format).toBe('webp'));
  });
  ...
  it('autoRun=true 时,inputId 出现触发 runWorkflow(quality=80)', async () => {
    ...
    const { rerender } = renderHook(() => useImageCompress());
  ...
  it('onComplete 在 outputBlob 变化时触发(且不重复)', async () => {
    ...
    const { rerender } = renderHook(() => useImageCompress({ onComplete }));
  ...
  it('run() 手动触发执行(autoRun=false 时仍可调用)', async () => {
    setMockState({ inputId: 'asset-1' });
    const { result } = renderHook(() => useImageCompress({ autoRun: false }));
    await act(async () => { await Promise.resolve(); });
  ...
  it('W23:传入 plugins 数组时,原样透传给 useImageTool(支持三方组合 audio/pdf/video 插件)', () => {
    ...
    const fakeImagePlugin = { config: { name: 'image-tools', version: '0.0.0', capabilities: [] }, install: () => {} };
    const fakeAudioPlugin = { config: { name: 'audio-tools', version: '0.0.0', capabilities: [] }, install: () => {} };
