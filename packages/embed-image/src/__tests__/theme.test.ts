...
import { themeToCssVars, type EmbedTheme } from '../theme';
...
describe('themeToCssVars', () => {
  ...
  it('primary 字段生成 --lokvis-primary', () => {
    const result = themeToCssVars({ primary: '#00ff00' } as EmbedTheme) as Record<string, string>;
  ...
  it('空字符串字段被忽略(回落到 global.css 默认值)', () => {
    const result = themeToCssVars({ primary: '' } as EmbedTheme) as Record<string, string>;
    expect(result['--lokvis-primary']).toBeUndefined();
  });

  it('null 字段被忽略', () => {
    // EmbedTheme 类型层面 primary 为 string | undefined,不允许 null。
    // 但运行时 JSON 注入可能产生 null,themeToCssVars 内部用 !== null 防御。
    // 用 JSON.parse 构造 null 值(JSON.parse 返回 any,as EmbedTheme 单次
    // 断言合规),避免 as unknown as 双断言(AGENTS.md 禁止)。
    const theme = JSON.parse('{"primary":null}') as EmbedTheme;
    const result = themeToCssVars(theme) as Record<string, string>;
    expect(result['--lokvis-primary']).toBeUndefined();
  });
});
