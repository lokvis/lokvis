 ...
 *
 * 来源:packages/capability/manifests/pdf.manifest.json
 * 重跑:pnpm codegen
 *
 * 与手写版本 packages/capability/src/presets/pdf.ts 逐字段对应;
...
export const PDF_COMPRESS: Capability = {
  ...
  outputTypes: ['pdf'],
  params: [
    { name: 'level', type: 'number', default: 6, min: 0, max: 9, description: '压缩级别 (0-9)' },
  ],
  performance: 'medium',
  batchable: true,
};
...
export const PDF_WATERMARK: Capability = {
  ...
  params: [
    ...
    { name: 'opacity', type: 'number', default: 0.3, min: 0, max: 1 },
...
export const PDF_ADD_PAGE_NUMBERS: Capability = {
  ...
  params: [
    ...
    },
    {
      ...
      type: 'string',
      default: '{n}',
      description: 'Page number format template ({n} = page number, {total} = total pages)',
    },
    { name: 'startFrom', type: 'number', default: 1, min: 1, description: 'Starting page number' },
    { name: 'fontSize', type: 'number', default: 12, min: 1, description: 'Font size in points' },
    { name: 'color', type: 'color', default: '#000000', description: 'Text color' },
  ],
  performance: 'fast',
  batchable: false,
};

/** 所有内置 pdf 能力预设(由 codegen 从 manifest 生成) */
