/**
 * QuickI18nProvider + locale prop i18n 解耦单测(Task 2)。
 *
 * 测试覆盖:
 *   - QuickI18nProvider 注入 locale:Layer 2 默认 UI 使用 Provider 的 locale
...
import { QuickI18nProvider, useQuickI18nContext } from '../i18n/QuickI18nProvider';
...
describe('QuickI18nProvider i18n 解耦', () => {
  beforeEach(() => {
    setMockState();
  });
  ...
  it('locale prop 在无 Provider 时生效(优先级 1 > 3/4 自动检测)', () => {
    render(<ImageQuickCompress locale="de" />);
  ...
  it('useQuickI18nContext 在 Provider 内返回 context value', () => {
    ...
    render(
      <QuickI18nProvider locale="fr" translations={{ foo: { fr: 'bar' } }}>
        <Probe />
      </QuickI18nProvider>
    );
