/**
 * useQuickWatermark — 图片一键加水印的纯逻辑 hook(Layer 0),无 UI。
 *
 * 行为:
...
export const WATERMARK_PRESETS: Record<WatermarkPreset, WatermarkPresetConfig> = {
  /** 右下角小字 */
  'small-br': {
...
/** useQuickWatermark 选项(扩展 UseQuickActionOptions) */
export interface UseQuickWatermarkOptions extends UseQuickActionOptions<WatermarkPreset> {
  /** 初始水印文字,默认 'Lokvis' */
  initialText?: string;
}

/** useQuickWatermark 返回值 */
export interface UseQuickWatermarkResult {
  // ─── 状态 ───
  ready: boolean;
 ...
 */
export function useQuickWatermark(
  options?: UseQuickWatermarkOptions
): UseQuickWatermarkResult {
  const {
  ...
  const runWatermark = useCallback(
    async (nextPreset: WatermarkPreset, nextText: string): Promise<void> => {
      const config = WATERMARK_PRESETS[nextPreset];
