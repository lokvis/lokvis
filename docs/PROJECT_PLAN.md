...
| 13.2 | API Reference 自动生成（tsdoc → typedoc） | P0 | 8h | ✅ | `apps/docs/src/content/docs/api/*`（427 页,9 个核心包） |
| 13.3 | Guides 补充：嵌入 SDK / 写第一个插件 / 自定义 Workspace / CLI 自动化 | P0 | 12h | ✅ | `apps/docs/src/content/docs/guides/*`（4 篇:embed-sdk / write-first-plugin / custom-workspace / cli-automation,build 输出 442 页） |
| 13.4 | Architecture 深度文：Runtime/Engine/Capability/Plugin 四层 | P0 | 6h | ✅ | `apps/docs/src/content/docs/architecture/*`（4 篇:runtime / engine / capability / plugin,build 输出 446 页） |
| 13.5 | README 根目录重写：介绍/架构图/快速开始/贡献指南 | P0 | 4h | ✅ | `README.md`(W12.5 已完成) |
| 13.6 | LICENSE 审计：确认 MIT + 第三方 WASM 协议清单 | P0 | 4h | ✅ | `THIRD_PARTY_LICENSES.md`(W12.6 已完成) |

### W14 · `apps/playground` 升级（40h）

| ID | 任务 | 优先级 | 估时 | 状态 | 产出 |
|---|---|---|---|---|---|
| 14.1 | Playground 极简壳：`PlaygroundLayout.astro` + nav 极简化 | P0 | 6h | ✅ | `apps/playground/src/layouts/PlaygroundLayout.astro`(BaseLayout 重命名 + nav 折叠组 + 3 占位页 sdk/plugin/workflow) |
| 14.2 | `SdkDemo.tsx`：createLokvis() 基础调用 demo | P0 | 6h | ✅ | `apps/playground/src/components/SdkDemo.tsx`(3 种构造配置对比:默认 / 预加载插件 / 自定义 RuntimeConfig) |
| 14.3 | `RuntimeDemo.tsx`：runtime.executor 调用 demo | P0 | 6h | ✅ | `apps/playground/src/components/RuntimeDemo.tsx`(W5 已创建,涵盖 importAsset/capabilities/eventBus) |
| 14.4 | `PluginDemo.tsx`：loadPlugin() 调用 demo | P0 | 6h | ✅ | `apps/playground/src/components/PluginDemo.tsx`(4 种加载场景:imageTools / devTools / 自定义 definePlugin / 重复加载错误路径) |
| 14.5 | `WorkflowDemo.tsx`：5 步 workflow demo | P0 | 6h | ✅ | `apps/playground/src/components/WorkflowDemo.tsx`(resize → compress → rotate → watermark → convert 5 步线性管道) |
| 14.6 | `McpManifestDemo.tsx`：toMcpManifest() 输出展示 | P0 | 6h | ✅ | `apps/playground/src/components/McpManifestDemo.tsx`(W5 已创建) |
| 14.7 | Playground 部署配置（playground.lokvis.dev） | P0 | 4h | ✅ | Cloudflare Pages 项目(`astro.config.mjs` site/base + `deploy-playground.yml` workflow + `_headers` COOP/COEP) |

### W15 · PWA 基础能力（仅作用于 apps/playground，40h）

| ID | 任务 | 优先级 | 估时 | 状态 | 产出 |
|---|---|---|---|---|---|
| 15.1 | Service Worker 预缓存：playground shell + top 5 engine | P0 | 6h | ✅ | `apps/playground/public/sw.js`(`lokvis-precache-v15.1.0` cache + `PRECACHE_URLS` shell + `register-sw.ts` 注册器 updateViaCache:'none' + hourly update polling + controllerchange 防循环 reload) |
| 15.2 | WASM 懒加载：用户触发工具才加载，prefetchOnHover | P0 | 6h | ✅ | `packages/engine-image/src/lazy.ts`(`loadEngineModule` 同步函数保留 Promise identity 缓存 + `prefetchOnHover` mouseenter/focus once 监听 + `getLoadStatus`/`getLoadStatusByCapability` + `TOP5_MODULES`/`CAPABILITY_TO_MODULE` 映射) |
| 15.3 | WASM immutable 缓存 + 失败重试 + 备用 CDN | P0 | 4h | ✅ | sw.js `handleEngineAsset`(cache-first + `fetchWithRetry` 3 次指数退避 200ms*i + `lokvis-engine-v15.1.0` immutable cache + `ENGINE_FALLBACK_CDNS` 占位数组) |
| 15.4 | `beforeinstallprompt` 捕获 + 自定义安装提示 UI | P0 | 6h | ✅ | `apps/playground/src/components/InstallPrompt.tsx`(idle/prompting/installed/dismissed 四态 + iOS Safari "Share → Add to Home Screen" 引导 + 7 天 localStorage dismiss 冷却 + `appinstalled` 2s 成功 toast) + `toolkit/pwa-utils.ts`(纯函数 isStandalone/isIosSafari/isDismissedInCooldown) |
| 15.5 | 离线状态指示：`navigator.onLine` + 事件 | P0 | 4h | ✅ | `apps/playground/src/components/StatusBar.tsx`(在线绿点 opacity 0.6 / 离线红点脉冲 + "Offline" 标签,点击跳 `/offline.html`)+ `pwa-utils.ts` `isOnline()`/`subscribeOnlineStatus()` SSR 安全 |
| 15.6 | 离线 fallback 页 | P0 | 2h | ✅ | `apps/playground/public/offline.html`(品牌色卡片 + "You are offline" 脉冲徽章 + Try again 按钮 + `online` 事件自动 reload) |
| 15.7 | PWA 安装后预加载 top 5 engine（后台静默） | P0 | 4h | ✅ | `apps/playground/src/toolkit/engine-preload.ts`(双通道:主线程 `preloadTop5Engines()` 内存缓存 + SW `postMessage({type:'PRELOAD_TOP5_ENGINES'})` HTTP 缓存,任一通道失败互不影响 + `setupEnginePreloadOnInstalled` appinstalled 触发 + `lokvis:engines-preloaded-at` localStorage flag) |
| 15.8 | 首次加载 Engine 介绍动画 + 进度条 + 预估时间 | P0 | 6h | ✅ | `apps/playground/src/components/EngineLoader.tsx`(conic-gradient halo 旋转 + 5 模块进度网格 + Skip 按钮 + 250ms fade out + 100ms 轮询 `getLoadStatus` + 观测速率预估剩余时间 `(elapsedMs/progress)-elapsedMs` + `lokvis:engine-loader-seen` 首次访问 flag) |
| 15.9 | 缓冲 | P0 | 2h | ✅ | —(缓冲未消耗,W15.1-W15.8 全部按预算完成,4h 缓冲转入 review 储备) |
...
| 2026-07-04 | 13.1 | ✅ 完成 | 3h | 全量 18 个 `@lokvis/*` 包 `package.json` 审计 + changeset 配置修复。**审计发现与修复**:① `.changeset/config.json` `baseBranch` 从 `"main"` 改为 `"dev"`(项目 base branch 是 dev);② 全部 18 个 packages 补 `sideEffects` 字段——UI 包(ui-core/ui-react)加 `"sideEffects": ["**/*.css"]` 保护 CSS tree-shaking,其余 16 包加 `"sideEffects": false`;③ 3 个核心包版本统一:`@lokvis/sdk` / `@lokvis/ui-react` / `@lokvis/ui-core` 从 `0.1.1-beta.0` 升至 `0.2.0-beta.0`(与 schema/runtime/capability/plugin-image/engine-image 对齐);④ `@lokvis/cli` 补顶级 `module` + `types` 字段(与其它 17 包一致);⑤ `apps/docs` + `apps/playground` 补 `"license": "MIT"` + `"sideEffects": false`。审计确认:18 包 exports 结构一致(types+import)、files 一致(dist+src+排除测试)、publishConfig 一致(public access)、license 全部 MIT。验证:typecheck 36/36 ✅、test 777/777 ✅、build 20/20 ✅。W13.5(README)+ W13.6(LICENSE)已在 W12.5/W12.6 完成,标记 ✅ |
| 2026-07-05 | 13.2 | ✅ 完成 | 6h | API Reference 自动生成(TypeDoc + starlight-typedoc)。**新增**:① `apps/docs/package.json` 加 devDependencies `typedoc ^0.28.19` + `typedoc-plugin-markdown ^4.12.0` + `starlight-typedoc ^0.23.0`;② `apps/docs/typedoc.json` 配置(entryPointStrategy=resolve,9 个核心包入口:schema/runtime/sdk/capability/plugin-sdk/plugin-image/engine-image/ui-react/ui-core,excludePrivate/Protected/Internal/Externals,categorizeByGroup,sort=source-order,tsconfig 指向 `../../tsconfig.typedoc.json`);③ `tsconfig.typedoc.json` 根目录新建(paths 映射 9 个 @lokvis/* 包到 src,include 9 包源码,exclude 测试文件);④ `apps/docs/astro.config.mjs` 集成 `starlightTypeDoc` 插件(sidebar collapsed=true,label='API Reference');⑤ `apps/docs/src/content.config.ts` 修复——补 `schema: docsSchema()`(原未应用 Starlight schema,导致 TypeDoc 生成页面无 `draft: false` 字段被 production build 过滤);⑥ `.gitignore` 加 `apps/docs/src/content/docs/api/`(生成产物不入库);⑦ `apps/docs/package.json` clean 脚本加 `src/content/docs/api`。**验证**:427 个 API markdown 页自动生成(9 包 × 平均 ~47 页),`pnpm build` 成功 438 页(原 11 + 427 API),Pagefind 索引 438 HTML。tsdoc 注释覆盖率良好(核心包 sdk/runtime/schema 已有完整 JSDoc + @example + @public 标记),无需补充。 |
| 2026-07-05 | 13.3 | ✅ 完成 | 4h | Guides 补充 4 篇,新建 `apps/docs/src/content/docs/guides/` 目录。① `embed-sdk.md`(嵌入 SDK guide):React 19 + Vite 主机应用嵌入 `<Workspace />` 完整示例 + Workspace 6 props 表 + 内部 `useLokvis` 初始化流程 + COOP/COEP 配置;② `write-first-plugin.md`(写第一个插件 guide):从零构建 `@lokvis/blur-image` 插件(capability 声明 → engine 操作 Blob→Blob 签名 `Record<string, any>` + AGENTS.md 禁双断言说明 → `definePlugin` + `createBlobCapabilityImpl` 装配 → loadPlugin 加载 → workflow 调用 + PluginContext 9 API 表 + stub 引擎处理 + 权限 + Vitest 测试);③ `custom-workspace.md`(自定义 Workspace guide):无 `@lokvis/ui-react` 仅 `@lokvis/sdk` + `@lokvis/plugin-image` 构建最小图像工作台(HTML + TS,importAsset → buildResizeWorkflow → run → exportAsset)+ `LokvisError` 错误体系(`fromLokvisError` 归一 + `DegradationRejectedError` instanceof 窄化 + `STORAGE_QUOTA_EXCEEDED` context 守卫)+ 自定义 vs `<Workspace />` 选择表;④ `cli-automation.md`(CLI 自动化 guide):Node.js 中 `runCLI(['capabilities'])` 能力查询 + `validateWorkflow` 工作流结构校验(Zod `SafeParseReturnType` shape: `result.success` + `result.error.issues`)+ Node 限制说明(Canvas/createImageBitmap 不可用,图像能力需 Playwright headless 浏览器)+ 6 命令表。**Sidebar**:`astro.config.mjs` 加 `Guides` 折叠组(4 项,Workflows 与 SDK 之间)。**Index**:`index.md` Next Steps 加 4 个 guides 链接。**验证**:`pnpm build` 成功 442 页(原 438 + 4 guides),Pagefind 索引 442 HTML,4 个 guides 路径正确生成(`guides/embed-sdk/` / `guides/write-first-plugin/` / `guides/custom-workspace/` / `guides/cli-automation/`) |
| 2026-07-05 | 13.4 | ✅ 完成 | 5h | Architecture 深度文 4 篇,新建 `apps/docs/src/content/docs/architecture/` 目录(原 `architecture.md` 保留为 Overview)。① `runtime.md`(@lokvis/runtime 深度文):模块映射(16 源文件)+ `LokvisRuntime` 接口契约 5 大领域(Asset / Workflow / History / Capability / MCP)+ `createRuntime()` 构造流程 + 5 子系统详解(AssetStore OPFS→IDB→Memory 三级 fallback + 配额 wrap + runExclusive 串行化 / CapabilityRegistry resolve 流程 stub 过滤 → preferredEngine → strategy / WorkflowExecutor 线性 DAG 执行 + node:finished → history 追加 / HistoryStack cursor-based undo/redo + LRU eviction 32 上限 + IDB 持久化 dirtyDuringLoad 守卫 / MemoryGuard 4 级压力 low/elevated/high/critical + pickDegradation L1-L4 阶梯)+ Worker 隔离协议 + 版本化与 4 个扩展钩子(`_getAssetStore` / `_getCapabilityRegistry` / `_getMemoryGuard` / `_registerMetadataReader`);② `engine.md`(@lokvis/engine-image 深度文):模块映射(types.ts / canvas-engine.ts / adapter.ts / worker-adapter.ts / operations/)+ `ImageEngineAdapter` 契约(decode / encode / isSupported)+ 操作函数签名 `(blob, params: Record<string, any>, signal?) => Promise<Blob>` + Canvas 引擎实现(createImageBitmap + OffscreenCanvas + convertToBlob)+ Streaming 类型(ImageTile / ImageChunk / StreamingImageOperation / StreamingImageEngineAdapter 供未来 WASM)+ Worker 集成(startImageWorker + dispatchImageMethod + Map<id, AbortController>)+ PNG pHYs chunk DPI 元数据 + stub 引擎模式;③ `capability.md`(@lokvis/capability + CapabilityRegistry 深度文):模块映射(names.ts / helpers.ts / presets/)+ `Capability` 形状(name / inputTypes / outputTypes / params / performance / batchable / mcpExposure)+ `CapabilityParam` 8 类型 + Phase 1 能力目录表(image ✅ / asset ✅ / developer ✅ / video stub / pdf stub)+ 11 个 helper 函数 + 63 平台预设 5 分类 + CapabilityRegistry 类 resolve 流程(filter stub → preferredEngine → strategy)+ `CapabilityImplementation` 接口 + 3 层 workflow 校验 + MCP manifest 生成(`capabilityParamsToJsonSchema`);④ `plugin.md`(@lokvis/plugin-sdk 深度文):模块映射 + `Plugin` 对象(`{config, install}`)+ 插件生命周期图 + `PluginContext` 受限 Runtime 视图(8 成员)+ `createBlobCapabilityImpl` 工厂(5 步样板封装)+ 完整示例(plugin-image ~70 行)+ `MetadataReader` 查询式操作例外(EXIF)+ stub plugin 模式 + 权限(Alpha 阶段 advisory)+ panels + Vitest 测试模式。**Sidebar**:`astro.config.mjs` 把原单条 `Architecture` 改为折叠组(Overview + 4 子项,放在 Getting Started 之后)。**验证**:`pnpm build` 成功 446 页(原 442 + 4 architecture),Pagefind 索引 446 HTML,4 个深度文路径正确生成(`architecture/runtime/` / `architecture/engine/` / `architecture/capability/` / `architecture/plugin/`) |
| 2026-07-05 | 14.1 | ✅ 完成 | 2h | Playground 极简壳升级。① `BaseLayout.astro` → `PlaygroundLayout.astro`(`git mv` 重命名,匹配 W14.1 任务交付物路径);② nav 极简化:`NavGroup` 接口加 `collapsed?: boolean` 字段,Demos 组(`collapsed: false`)展开 / Tools W5 组(`collapsed: true`)折叠,sidebar 改用 `<details>` + `<summary>`(native HTML 折叠,无需 JS),summary 加 `group-open:rotate-90` 旋转箭头指示器;③ Demos 组新增 3 个 SDK 演示入口(sdk / plugin / workflow,占位 desc 简短英文)+ 现有 6 个 demo desc 简化;④ 创建 3 个占位页 `sdk.astro` / `plugin.astro` / `workflow.astro`(各含"W14.2/4/5 demo loading…"提示,等 W14.2/4/5 实现后替换为 React 组件);⑤ 全量更新 9 个文件引用(`pages/index.astro` / `runtime.astro` / `image.astro` / `mcp.astro` / `components.astro` / `history.astro` + `layouts/ToolLayout.astro` + 注释 `toolkit/sentry.ts` + `components/tools/seo.ts`)`BaseLayout` → `PlaygroundLayout`。**验证**:`pnpm build --filter @lokvis/playground` 成功 17 页(原 14 + 3 占位 sdk/plugin/workflow) |
| 2026-07-05 | 14.2 | ✅ 完成 | 2h | `SdkDemo.tsx` —— `createLokvis()` 基础调用 demo。新建 `apps/playground/src/components/demos/SdkDemo.tsx`(264 行),演示 SDK 唯一入口 `createLokvis()` 的 3 种典型构造方式:① 默认无插件 `createLokvis()` —— 空运行时,0 capabilities;② 预加载插件 `createLokvis({ plugins: [imageToolsPlugin(), devToolsPlugin()] })` —— image + dev 能力;③ 自定义 RuntimeConfig(`enableOpfs: false` / `enableIndexedDB: false` / `storageQuota: 256MB` / `engineStrategy: 'first'` / `memoryBudget: 128MB`)—— 内存态运行时。**UI**:3 列并排对比 + 每列展示 Options JSON / Runtime Snapshot(version / status / isPro / capabilitiesCount)/ Capability Names 标签云 + 顶部 "▶ Run all 3" 按钮顺序构造 + 构造耗时计时 + 错误展示。**清理**:每次重跑前 `Promise.all(runtimes.map(rt => rt.cancel('all')))` 释放上一轮 runtime;组件卸载时同样清理。**集成**:`sdk.astro` 占位页替换为 `<SdkDemo />` + `ErrorBoundary` 包裹。**验证**:`pnpm build --filter @lokvis/playground` 成功 17 页 |
| 2026-07-05 | 14.3 | ✅ 已存在 | 0h | `RuntimeDemo.tsx` —— `runtime` 基础调用 demo。文件 `apps/playground/src/components/demos/RuntimeDemo.tsx` 已在 W5 阶段创建,涵盖 W14.3 任务全部要点:① `createLokvis({ plugins: [imageToolsPlugin(), devToolsPlugin()] })` 工厂构造;② `runtime.importAsset({ kind: 'file', file })` 资产导入(File 多选)+ `listAssets()` 列表 + `removeAsset(id)` 删除;③ `runtime.capabilities()` 能力声明查询(展示 name + description);④ `runtime.eventBus.onAny()` 事件订阅(实时日志输出)。**UI**:3 列布局(Capabilities / Assets / Event Log)+ Upload 按钮 + Clear 日志按钮 + 200 条日志环形缓冲。**集成**:`runtime.astro` 已使用 `<RuntimeDemo />` + `ErrorBoundary` 包裹。本任务无代码变更,仅 PROJECT_PLAN 状态标记 |
| 2026-07-05 | 14.4 | ✅ 完成 | 3h | `PluginDemo.tsx` —— `loadPlugin()` 动态扩展 demo。新建 `apps/playground/src/components/demos/PluginDemo.tsx`(358 行),演示 SDK 运行时动态加载 API。**关键差异**:`createLokvis({ plugins })` 构造时一次性预加载 vs `loadPlugin()` runtime 已存在后动态注入(用户点击触发)。**4 种加载场景**:① `imageToolsPlugin`(npm `@lokvis/plugin-image`)→ 9 image capabilities;② `devToolsPlugin`(npm `@lokvis/plugin-dev`)→ developer capabilities;③ 内联 `definePlugin({...})` 自定义插件(浏览器内构造,仅注册 `demo.echo` 能力声明,无操作实现);④ `imageToolsPlugin` 重复加载(预期失败 → 错误路径,演示 CapabilityRegistry 重复注册保护)。**UI**:2 列布局(左 Plugins 列表 4 项 + Load 按钮 + 加载耗时计时 + loaded/failed 状态徽章 + 错误展开;右 Capabilities 列表 + Event Log 200 条环形缓冲)。**事件流**:`eventBus.onAny` 监听 `plugin:loaded` 事件实时输出 `name@version`。**清理**:每次加载用 `runtimeRef.current` 引用 + 卸载时 `rt.cancel('all')`。**依赖**:`apps/playground/package.json` 加 `@lokvis/plugin-sdk: workspace:*`(原本仅通过 plugin-image / plugin-dev 传递依赖,本次显式声明)。**集成**:`plugin.astro` 占位页替换为 `<PluginDemo />` + `ErrorBoundary` 包裹。**验证**:`pnpm install --no-frozen-lockfile` + `pnpm build --filter @lokvis/playground` 成功 17 页 |
| 2026-07-05 | 14.5 | ✅ 完成 | 3h | `WorkflowDemo.tsx` —— 5 步 workflow demo。新建 `apps/playground/src/components/demos/WorkflowDemo.tsx`,演示 5 步线性图像处理管道。**管道**:resize(width:800,height:600,fit:cover) → compress(quality:80) → rotate(angle:90) → watermark(text:'Lokvis',position:bottom-right,fontSize:24,color:#ffffff,opacity:0.8) → convert(format:image/webp)。**Workflow 对象**:5 个 node(node-1..node-5,type:'transform')+ 4 个 edge 线性连接 + input(image)+ output(image/webp)。**UI**:2 列布局(左 Pipeline 可视化 5 个步骤框 + 参数 JSON + Upload/Run/Cancel 按钮 + 进度条 done/total;右 Event Log 200 条环形缓冲 + Results 区域显示输出下载链接与预览)。**执行流**:`importAsset` → `run(workflow, [asset.id])` → 完成后 `exportAsset(outputs[0])` 获取 Blob → Object URL 下载链接。**事件**:`eventBus.onAny` 捕获 workflow:started / node:started / node:finished / node:failed / workflow:completed / workflow:cancelled。**清理**:卸载时 revoke Object URL + unsub eventBus + `rt.cancel('all')`。**集成**:`workflow.astro` 占位页替换为 `<WorkflowDemo />` + `ErrorBoundary` 包裹。**验证**:`pnpm build --filter @lokvis/playground` 成功 17 页 |
| 2026-07-05 | 14.6 | ✅ 已存在 | 0h | `McpManifestDemo.tsx` —— `toMcpManifest()` 输出展示。文件 `apps/playground/src/components/demos/McpManifestDemo.tsx` 已在 W5 阶段创建,涵盖 W14.6 任务全部要点:展示 runtime 的 `toMcpManifest()` API 输出(McpManifest 结构:tools / resources / prompts 三部分)。**集成**:`mcp.astro` 已使用 `<McpManifestDemo />` + `ErrorBoundary` 包裹。本任务无代码变更,仅 PROJECT_PLAN 状态标记 |
| 2026-07-05 | 14.7 | ✅ 完成 | 2h | Playground 部署配置(playground.lokvis.dev)。① `apps/playground/astro.config.mjs` 更新:`site` 从 `https://lokvis.com` 改为 `https://playground.lokvis.dev`,`base` 从 `/playground` 改为 `/`(subdomain 根部署,无需路径前缀);PlaygroundLayout.astro 使用 `import.meta.env.BASE_URL` 动态构造 href,base 变更后所有导航链接自动适配;② 新建 `.github/workflows/deploy-playground.yml` Cloudflare Pages 部署 workflow(push 到 dev/main 分支且 apps/playground 或 packages 有变更时触发 + workflow_dispatch 手动触发 + `cloudflare/pages-action@v1` + 项目名 `lokvis-playground` + dev 分支预览部署 / main 分支生产部署 + 前置条件文档化 CLOUDFLARE_API_TOKEN / CLOUDFLARE_ACCOUNT_ID Secrets);③ `apps/playground/public/_headers` 已含 COOP/COEP 安全头(Cross-Origin-Opener-Policy: same-origin + Cross-Origin-Embedder-Policy: require-corp,Cloudflare Pages 自动识别 `_headers` 文件,启用 SharedArrayBuffer 供 image engine 使用)。**验证**:`pnpm build --filter @lokvis/playground` 成功 17 页,dist 输出路径正确(`/sdk` / `/runtime` / `/plugin` 等根相对路径) |
| 2026-07-05 | 15.1+3+6 | ✅ 完成 | 8h | W15 SW 基础设施 + 离线 fallback 三任务合并落地。① `apps/playground/public/sw.js`(13.7KB)Service Worker:三套缓存(`lokvis-precache-v15.1.0` shell / `lokvis-runtime-v15.1.0` 静态资源 / `lokvis-engine-v15.1.0` engine 模块 immutable),`PRECACHE_URLS` 预缓存 shell(`/` / `/offline.html` / `/manifest.webmanifest` / `/icon.svg` / `/icon-maskable.svg`),install skipWaiting + activate clients.claim + 旧缓存清理;fetch 路由:导航请求 network-first 降级 `/offline.html`,静态资源 stale-while-revalidate,engine 资源 `handleEngineAsset`(cache-first + `fetchWithRetry` 3 次指数退避 200ms*i + `ENGINE_FALLBACK_CDNS` 占位备用 CDN 数组);`PRELOAD_TOP5_ENGINES` postMessage 处理器 SW 端 HTTP 预缓存;`notifyClients()` 通过 `postMessage` 通知所有 client;② `apps/playground/src/toolkit/register-sw.ts` SW 注册器:dev + 非安全上下文跳过,`updateViaCache: 'none'`,每小时 `registration.update()` 轮询,`controllerchange` 监听 + `window.__lokvisSWReloaded` 防循环 reload,返回 `{ registration, triggerSkipWaiting }`;③ `apps/playground/public/offline.html` 离线 fallback 页:品牌色卡片 + "You are offline" 脉冲徽章 + "Try again" 按钮 + `online` 事件自动 reload;④ `apps/playground/public/manifest.webmanifest` PWA manifest:standand 显示模式,品牌色 `#0F172A` 主题,SVG 图标(maskable + 任意形状);⑤ `apps/playground/public/icon.svg` + `icon-maskable.svg` 品牌图标(渐变 logo);**Review 修复**:`handleEngineAsset` 原引用外层 FetchEvent 的 `event` 变量越界,改为 `handleEngineAsset(event, request)` 显式传参。**验证**:`pnpm build --filter @lokvis/playground` 成功,manifest + sw.js + offline.html 静态产物正确输出 |
| 2026-07-05 | 15.2 | ✅ 完成 | 4h | `packages/engine-image/src/lazy.ts`(7.8KB)引擎模块懒加载基础设施。**关键设计**:`loadEngineModule<T>(name)` 故意不声明 `async` —— async 函数会包装返回值为新 Promise,破坏缓存 Promise identity;改为同步函数,直接返回 `moduleCache` 中缓存的 Promise 引用(首次调用动态 import + 状态机 idle→loading→loaded/error,error 时 `moduleCache.delete(name)` 允许重试)。**导出 API**:① `EngineModuleName` 类型(7 个模块:resize/compress/convert/crop/watermark/rotate/filter);② `LoadStatus` idle/loading/loaded/error;③ `CAPABILITY_TO_MODULE` capability → module 反查表;④ `TOP5_MODULES` 数组(resize/compress/convert/watermark/rotate);⑤ `loadEngineModule<T>(name)` 主 API;⑥ `prefetchEngineModule(name)` 预取(忽略错误,失败时清 cache);⑦ `preloadTop5Engines()` 批量预取 top 5;⑧ `getLoadStatus(name)` / `getLoadStatusByCapability(capability)` 状态查询;⑨ `prefetchOnHover(element, name)` / `prefetchOnHoverCapability(element, capability)` hover/focus `{ once: true }` 监听器 + 返回 cleanup 函数;⑩ `_resetLazyCache()` 测试用。`packages/engine-image/src/index.ts` 加 `export * from './lazy.js'`。**测试**:`packages/engine-image/src/__tests__/lazy.test.ts` 14 个(jsdom 环境,`vi.mock` 模拟 7 个 importer,覆盖 loadEngineModule 缓存 Promise identity + 状态机 + error 重试 + prefetchOnHover mouseenter/focus 触发 + CAPABILITY_TO_MODULE 映射 + preloadTop5Engines 并发 + getLoadStatus 查询)。**Bug 修复**:测试在 vitest 默认 node 环境失败(`document is not defined`),加 `// @vitest-environment jsdom` 注解。**验证**:typecheck ✅、test ✅(新增 14)、build ✅ |
| 2026-07-05 | 15.4+5 | ✅ 完成 | 8h | W15.4 InstallPrompt + W15.5 StatusBar 合并落地。① `apps/playground/src/toolkit/pwa-utils.ts`(2.8KB)共享纯函数模块(供 UI 组件 + 测试消费,SSR 安全 try/catch):`DISMISS_KEY` / `DISMISS_COOLDOWN_MS=7d` / `isStandalone()`(matchMedia '(display-mode: standalone)' 或 navigator.standalone iOS)/ `isIosSafari()`(UA 含 Safari + 不含 CriOS/FxiOS/EdiOS)/ `isDismissedInCooldown(now)`(localStorage 读 timestamp + 7 天比较)/ `markDismissed(now)` / `clearDismissed()` / `isOnline()`(navigator.onLine,SSR 默认 true)/ `subscribeOnlineStatus(callback)`(online/offline 事件 + 返回 unsub);② `apps/playground/src/components/InstallPrompt.tsx`(10.2KB)PWA 安装提示组件:四态 'idle' / 'prompting' / 'installed' / 'dismissed',`beforeinstallprompt` 事件捕获 deferredPrompt + preventDefault,自定义 UI(图标 + 标题 + "Install App" 按钮 + "Not now" 关闭),iOS Safari 检测显示 "Share → Add to Home Screen" 引导,dismiss 写 localStorage 7 天冷却,`appinstalled` 事件 2s 成功 toast + SW 预加载触发;③ `apps/playground/src/components/StatusBar.tsx`(3.7KB)在线状态指示器:在线绿点 opacity 0.6(几乎不可见,避免干扰)/ 离线红点 pulse 动画 + "Offline" 文字,点击跳 `/offline.html`,client:only 避免 SSR 水合不匹配;④ `apps/playground/src/toolkit/__tests__/pwa-utils.test.ts` 17 个单测(覆盖 isStandalone 三路径 / isIosSafari UA 检测 4 场景 / dismiss 冷却边界 just-now/6d-23h/7d/8d / localStorage 抛异常容错 / online/offline 事件订阅与 unsub)。**集成**:PlaygroundLayout.astro `<head>` 加 manifest link + theme-color + apple-touch-icon + apple-mobile-web-app meta,body 顶部 `<InstallPrompt client:only="react" />` 浮层,header `<StatusBar client:only="react" />`。**验证**:typecheck ✅、test ✅(新增 17)、build ✅ |
| 2026-07-05 | 15.7 | ✅ 完成 | 4h | `apps/playground/src/toolkit/engine-preload.ts`(4.1KB)PWA 安装后双通道预加载编排。**核心函数** `preloadEngines()` 返回 `{ mainThread: Record<string,string>, sw: 'sent' \| 'skipped' \| 'failed' }`:① 主线程通道——调用 `@lokvis/engine-image` 的 `preloadTop5Engines()` 获得内存缓存(立即生效),返回每个 module 的 load status;② SW 通道——`navigator.serviceWorker.controller.postMessage({ type: 'PRELOAD_TOP5_ENGINES' })` 触发 SW 端 HTTP 缓存(供下次离线访问);两通道独立,任一失败不影响另一;③ 写入 `localStorage['lokvis:engines-preloaded-at']` 标记预加载时间。**辅助函数**:`setupEnginePreloadOnInstalled()`(订阅 `appinstalled` 事件 + 5s 延迟后触发 `preloadEngines`,失败 console.warn 不阻塞)、`isEnginesPreloaded()`(读 localStorage 标记)、`subscribeEnginePreloadProgress(onProgress, onDone)`(订阅 SW message `ENGINE_PRELOAD_PROGRESS` 推送进度)。**依赖**:`apps/playground/package.json` 加 `"@lokvis/engine-image": "workspace:*"`(原本通过 plugin-image 传递依赖,本次显式声明以便直接 import `preloadTop5Engines`)+ `pnpm install --no-frozen-lockfile`。**Bug 修复**:`subscribeEnginePreloadProgress` 原检查 `'serviceWorker' in navigator` 但 `navigator.serviceWorker` 在测试环境为 undefined 导致 `addEventListener` 抛错,改为 `!navigator.serviceWorker`。**测试**:`apps/playground/src/toolkit/__tests__/engine-preload.test.ts` 12 个单测(用 `vi.hoisted()` 创建 mock 函数引用避免 vi.mock 提升问题,覆盖 appinstalled 触发 setupEnginePreloadOnInstalled / preloadEngines 双通道主线程+SW 并行 / SW 不可用降级 / subscribeEnginePreloadProgress 无 controller 降级 / localStorage 持久化)。**集成**:PlaygroundLayout.astro `<script>` 调 `setupEnginePreloadOnInstalled()`(原 InstallPrompt 内 SW message 触发逻辑移出,职责单一)。**验证**:typecheck ✅、test ✅(新增 12)、build ✅ |
| 2026-07-05 | 15.8 | ✅ 完成 | 4h | `apps/playground/src/components/EngineLoader.tsx`(13.2KB)首次访问 Engine 加载引导动画。**触发条件** `shouldShow(force) = force || (!isStandalone && !hasSeenLoader)`——非 standalone 模式 + 首次访问(localStorage `lokvis:engine-loader-seen` flag),避免 PWA 已安装用户重复看到。**UI**:全屏品牌色背景 + conic-gradient halo 旋转动画 + Lokvis logo + "Preparing your image engines" 标题 + 5 模块进度网格(每个 module 一行:图标 + 名称 + 状态 idle/loading/loaded/error → 灰/蓝脉冲/绿✓/红⚠)+ 横向进度条(loaded=1, error=0.5)+ 预估剩余时间文字 + Skip 按钮。**轮询**:`setTimeout` 每 100ms 调 `getLoadStatus(name)` 查询所有 module 状态,全部离开 loading 状态后等 500ms 完成 + 250ms fade out + 写 localStorage seen flag;组件 unmount 清 timer。**预估剩余时间公式**:`progress <= 0 ? ESTIMATED_TOTAL_MS(8s) : Math.max(0, Math.ceil(elapsedMs / progress) - elapsedMs)`——基于已观测速率反推总耗时,比固定线性插值更准确。**Props**:`force?: boolean`(强制显示,用于调试)+ `onComplete?: () => void`。**集成**:PlaygroundLayout.astro body 顶部 `<EngineLoader client:only="react" />`。**验证**:typecheck ✅、build ✅(无新增测试,纯 UI 组件 + 已有 pwa-utils/lazy/engine-preload 测试覆盖核心逻辑) |
| 2026-07-05 | 15.9 | ✅ 缓冲 | 0h | W15.9 缓冲未消耗。W15.1-W15.8 全部按预算完成(实际消耗 28h vs 预算 38h,节省 10h),剩余 12h 转入 PR review 储备 |
| 2026-07-05 | W15 | 🎉 收尾 | 28h | W15 PWA 基础能力 milestone 完成。typecheck ✅(36 tasks)、test ✅(820 测试,新增 43:lazy 14 + pwa-utils 17 + engine-preload 12)、build ✅(20 tasks)、覆盖率 lines 91.62% / branches 88.19% / functions 88.07% / statements 91.62%(远超阈值 60%/75%)。15.9 缓冲未消耗。**6 commits**:① `4a68ca2` W15.1+3+6 SW 基础设施 + offline fallback;② `ff94d22` W15.2 engine-image 懒加载;③ `3e6fa60` W15.4+5 InstallPrompt + StatusBar;④ `92d501f` W15.7 PWA 安装后预加载;⑤ `60f3b4e` W15.8 EngineLoader;⑥ (本提交)docs(w15) 状态标记。所有 commits 已 push 到 remote `feat/w12-alpha-milestone` |
...
| 日期 | 版本 | 变更 |
|---|---|---|
| 2026-07-05 | v5.4 | **W15.8 EngineLoader 首次访问 Engine 加载引导动画**:`apps/playground/src/components/EngineLoader.tsx`(13.2KB)。触发条件 `shouldShow(force) = force \|\| (!isStandalone && !hasSeenLoader)`——非 standalone + 首次访问(localStorage `lokvis:engine-loader-seen` flag)。UI:全屏品牌色 + conic-gradient halo 旋转 + Lokvis logo + 5 模块进度网格(idle/loading/loaded/error → 灰/蓝脉冲/绿✓/红⚠)+ 进度条(loaded=1, error=0.5)+ 预估剩余时间 + Skip 按钮。轮询 `setTimeout` 100ms 调 `getLoadStatus`,全部离开 loading 后 500ms 完成 + 250ms fade out + 写 seen flag。预估时间公式 `progress <= 0 ? 8s : Math.max(0, Math.ceil(elapsedMs / progress) - elapsedMs)` 基于已观测速率反推。集成:PlaygroundLayout body 顶部 `<EngineLoader client:only="react" />`。验证:typecheck ✅、build ✅ |
| 2026-07-05 | v5.3 | **W15.7 PWA 安装后预加载 top 5 engine 双通道**:`apps/playground/src/toolkit/engine-preload.ts`(4.1KB)。核心 `preloadEngines()` 返回 `{ mainThread, sw }`:① 主线程 `preloadTop5Engines()` 内存缓存(立即生效);② SW `postMessage({type:'PRELOAD_TOP5_ENGINES'})` HTTP 缓存(下次离线可用);两通道独立互不影响 + `localStorage['lokvis:engines-preloaded-at']` 标记。辅助:`setupEnginePreloadOnInstalled()`(appinstalled 事件 + 5s 延迟)、`isEnginesPreloaded()`、`subscribeEnginePreloadProgress(onProgress, onDone)`(订阅 SW `ENGINE_PRELOAD_PROGRESS` 消息)。依赖:`apps/playground/package.json` 加 `@lokvis/engine-image: workspace:*`。Bug 修复:`subscribeEnginePreloadProgress` `'serviceWorker' in navigator` 检查在 `navigator.serviceWorker` undefined 时仍抛错,改 `!navigator.serviceWorker`。测试 12 个(vi.hoisted 解决 vi.mock 提升 + 覆盖 appinstalled 触发 / 双通道并行 / SW 降级 / localStorage 持久化)。验证:typecheck ✅、test ✅(新增 12)、build ✅ |
| 2026-07-05 | v5.2 | **W15.4 + W15.5 InstallPrompt + StatusBar PWA 安装提示与离线状态**:① `toolkit/pwa-utils.ts`(2.8KB)SSR 安全纯函数:`DISMISS_KEY` / `DISMISS_COOLDOWN_MS=7d` / `isStandalone()`(matchMedia + iOS navigator.standalone)/ `isIosSafari()`(UA 含 Safari 不含 CriOS/FxiOS/EdiOS)/ `isDismissedInCooldown(now)` / `markDismissed(now)` / `clearDismissed()` / `isOnline()` / `subscribeOnlineStatus(callback)`;② `components/InstallPrompt.tsx`(10.2KB)四态 idle/prompting/installed/dismissed + `beforeinstallprompt` 捕获 deferredPrompt + iOS Safari "Share → Add to Home Screen" 引导 + 7 天 localStorage dismiss 冷却 + `appinstalled` 2s 成功 toast;③ `components/StatusBar.tsx`(3.7KB)在线绿点 opacity 0.6 / 离线红点 pulse + "Offline" 文字,点击跳 `/offline.html`。测试 17 个(isStandalone 三路径 / isIosSafari UA 4 场景 / dismiss 冷却边界 / localStorage 异常容错 / online/offline 事件)。集成:PlaygroundLayout `<head>` 加 manifest + theme-color + apple-touch-icon + apple-mobile-web-app meta,body 顶部 InstallPrompt 浮层,header StatusBar。验证:typecheck ✅、test ✅(新增 17)、build ✅ |
| 2026-07-05 | v5.1 | **W15.2 engine-image 懒加载基础设施**:`packages/engine-image/src/lazy.ts`(7.8KB)。关键设计:`loadEngineModule<T>(name)` 故意不声明 `async`(async 会包装新 Promise 破坏缓存 identity),改为同步函数返回 `moduleCache` 中 Promise 引用 + 状态机 idle→loading→loaded/error,error 时 `moduleCache.delete(name)` 允许重试。导出:`EngineModuleName`(7 模块 resize/compress/convert/crop/watermark/rotate/filter)/ `LoadStatus` / `CAPABILITY_TO_MODULE` 反查表 / `TOP5_MODULES` / `loadEngineModule` / `prefetchEngineModule` / `preloadTop5Engines` / `getLoadStatus` / `getLoadStatusByCapability` / `prefetchOnHover`(mouseenter/focus `{ once: true }` + 返回 cleanup)/ `prefetchOnHoverCapability` / `_resetLazyCache`。`index.ts` 加 `export * from './lazy.js'`。测试 14 个(jsdom 环境 + vi.mock 模拟 7 importer,覆盖缓存 Promise identity + 状态机 + error 重试 + hover/focus 触发 + CAPABILITY_TO_MODULE 映射)。Bug 修复:测试在 node 环境失败,加 `// @vitest-environment jsdom` 注解。验证:typecheck ✅、test ✅(新增 14)、build ✅ |
| 2026-07-05 | v5.0 | **W15.1 + W15.3 + W15.6 SW 基础设施 + immutable 缓存 + 离线 fallback**:① `apps/playground/public/sw.js`(13.7KB)Service Worker:三套缓存 `lokvis-precache-v15.1.0`(shell)/ `lokvis-runtime-v15.1.0`(静态)/ `lokvis-engine-v15.1.0`(engine immutable),`PRECACHE_URLS` 预缓存 shell,install skipWaiting + activate clients.claim + 旧缓存清理,fetch 路由:导航 network-first 降级 `/offline.html`、静态 stale-while-revalidate、engine `handleEngineAsset`(cache-first + `fetchWithRetry` 3 次指数退避 200ms*i + `ENGINE_FALLBACK_CDNS` 备用 CDN 占位),`PRELOAD_TOP5_ENGINES` postMessage 处理器,`notifyClients()` postMessage 通知;② `toolkit/register-sw.ts`(3.4KB)SW 注册器:dev + 非安全上下文跳过,`updateViaCache: 'none'`,每小时 `registration.update()` 轮询,`controllerchange` + `window.__lokvisSWReloaded` 防循环 reload;③ `public/offline.html`(4.1KB)品牌色卡片 + "You are offline" 脉冲徽章 + Try again + `online` 事件自动 reload;④ `public/manifest.webmanifest` standalone 显示 + 品牌色 + SVG 图标;⑤ `public/icon.svg` + `icon-maskable.svg` 渐变 logo。Review 修复:`handleEngineAsset` 原引用外层 `event` 越界,改 `handleEngineAsset(event, request)` 显式传参。验证:`pnpm build --filter @lokvis/playground` 成功,manifest + sw.js + offline.html 正确输出 |
| 2026-07-05 | v4.6 | **W14.3/W14.4/W14.5/W14.6 Playground demos 收尾**:① W14.3 `RuntimeDemo.tsx` —— 已在 W5 创建(importAsset/capabilities/eventBus 3 列布局),标记 ✅ 已存在;② W14.4 `PluginDemo.tsx`(358 行)—— `loadPlugin()` 动态扩展 demo,4 种加载场景(imageTools / devTools / 自定义 definePlugin / 重复加载错误路径)+ 2 列布局(Plugins 列表 + Capabilities/Event Log)+ `eventBus.onAny` 监听 `plugin:loaded` 事件 + `@lokvis/plugin-sdk` 显式加入 playground deps;③ W14.5 `WorkflowDemo.tsx` —— 5 步线性管道 demo(resize→compress→rotate→watermark→convert)+ 2 列布局(Pipeline 可视化 + Event Log/Results)+ 完整执行流 importAsset→run→exportAsset→Object URL 下载 + workflow/node 事件订阅;④ W14.6 `McpManifestDemo.tsx` —— 已在 W5 创建,标记 ✅ 已存在。验证:`pnpm build --filter @lokvis/playground` 成功 17 页 |
| 2026-07-05 | v4.7 | **W14.7 Playground 部署配置(playground.lokvis.dev)**:① `apps/playground/astro.config.mjs` 更新:`site` `https://lokvis.com`→`https://playground.lokvis.dev`,`base` `/playground`→`/`(subdomain 根部署);② 新建 `.github/workflows/deploy-playground.yml` Cloudflare Pages 部署 workflow(push 到 dev/main + workflow_dispatch + `cloudflare/pages-action@v1` + 项目名 `lokvis-playground` + dev 预览 / main 生产 + CLOUDFLARE_API_TOKEN/CLOUDFLARE_ACCOUNT_ID Secrets 文档化);③ `_headers` 已含 COOP/COEP 安全头(Cloudflare Pages 自动识别,启用 SharedArrayBuffer)。验证:`pnpm build --filter @lokvis/playground` 成功 17 页,dist 根相对路径正确 |
| 2026-07-05 | v4.5 | **W14.2 SdkDemo.tsx**:`createLokvis()` 基础调用 demo。新建 `apps/playground/src/components/demos/SdkDemo.tsx`(264 行),演示 SDK 唯一入口 `createLokvis()` 的 3 种典型构造方式:① 默认无插件 → 0 capabilities;② 预加载 imageTools + devTools → image + dev 能力;③ 自定义 RuntimeConfig(enableOpfs:false / enableIndexedDB:false / storageQuota:256MB / engineStrategy:'first' / memoryBudget:128MB)→ 内存态运行时。UI:3 列并排对比 + Options JSON / Runtime Snapshot(version / status / isPro / capabilitiesCount)/ Capability Names 标签云 + 顶部 "▶ Run all 3" 按钮顺序构造 + 构造耗时计时 + 错误展示。清理:每次重跑前 `Promise.all(runtimes.map(rt => rt.cancel('all')))` 释放上一轮 runtime;组件卸载时同样清理。集成:`sdk.astro` 占位页替换为 `<SdkDemo />` + `ErrorBoundary` 包裹。验证:`pnpm build --filter @lokvis/playground` 成功 17 页 |
| 2026-07-05 | v4.4 | **W14.1 Playground 极简壳升级**:① `BaseLayout.astro` → `PlaygroundLayout.astro`(`git mv` 重命名,匹配 W14.1 任务交付物路径);② nav 极简化:`NavGroup` 接口加 `collapsed?: boolean` 字段,Demos 组(`collapsed: false`)展开 / Tools W5 组(`collapsed: true`)折叠,sidebar 改用 `<details>` + `<summary>`(native HTML 折叠,无需 JS),summary 加 `group-open:rotate-90` 旋转箭头指示器;③ Demos 组新增 3 个 SDK 演示入口(sdk / plugin / workflow,占位 desc 简短英文)+ 现有 6 个 demo desc 简化;④ 创建 3 个占位页 `sdk.astro` / `plugin.astro` / `workflow.astro`(各含"W14.2/4/5 demo loading…"提示,等 W14.2/4/5 实现后替换为 React 组件);⑤ 全量更新 9 个文件引用(`pages/index.astro` / `runtime.astro` / `image.astro` / `mcp.astro` / `components.astro` / `history.astro` + `layouts/ToolLayout.astro` + 注释 `toolkit/sentry.ts` + `components/tools/seo.ts`)`BaseLayout` → `PlaygroundLayout`。验证:`pnpm build --filter @lokvis/playground` 成功 17 页(原 14 + 3 占位 sdk/plugin/workflow) |
| 2026-07-05 | v4.3 | **W13.4 Architecture 深度文 4 篇**:新建 `apps/docs/src/content/docs/architecture/` 目录(原 `architecture.md` 保留为 Overview)。① `runtime.md` @lokvis/runtime 深度文(模块映射 16 源文件 + LokvisRuntime 接口契约 5 大领域 Asset/Workflow/History/Capability/MCP + createRuntime 构造流程 + 5 子系统详解:AssetStore OPFS→IDB→Memory 三级 fallback + 配额 wrap runExclusive 串行化 / CapabilityRegistry resolve 流程 stub→preferredEngine→strategy / WorkflowExecutor 线性 DAG + node:finished→history / HistoryStack cursor-based undo/redo + LRU eviction 32 上限 + IDB 持久化 dirtyDuringLoad 守卫 / MemoryGuard 4 级压力 + pickDegradation L1-L4 阶梯 + Worker 隔离协议 + 4 个扩展钩子 `_getAssetStore`/`_getCapabilityRegistry`/`_getMemoryGuard`/`_registerMetadataReader`);② `engine.md` @lokvis/engine-image 深度文(模块映射 types/canvas-engine/adapter/worker-adapter/operations + ImageEngineAdapter 契约 decode/encode/isSupported + 操作签名 `(blob, params: Record<string, any>, signal?) => Promise<Blob>` + Canvas 引擎 createImageBitmap+OffscreenCanvas+convertToBlob + Streaming 类型 ImageTile/ImageChunk/StreamingImageOperation 供未来 WASM + Worker 集成 startImageWorker+dispatchImageMethod+Map<id, AbortController> + PNG pHYs chunk DPI + stub 引擎模式);③ `capability.md` @lokvis/capability + CapabilityRegistry 深度文(模块映射 names/helpers/presets + Capability 形状 name/inputTypes/outputTypes/params/performance/batchable/mcpExposure + CapabilityParam 8 类型 + Phase 1 能力目录表 image ✅/asset ✅/developer ✅/video stub/pdf stub + 11 helper 函数 + 63 平台预设 5 分类 + CapabilityRegistry resolve 流程 filter stub→preferredEngine→strategy + CapabilityImplementation 接口 + 3 层 workflow 校验 + MCP manifest 生成 capabilityParamsToJsonSchema);④ `plugin.md` @lokvis/plugin-sdk 深度文(模块映射 + Plugin 对象 `{config, install}` + 插件生命周期图 + PluginContext 受限 Runtime 视图 8 成员 + createBlobCapabilityImpl 工厂 5 步样板封装 + 完整示例 plugin-image ~70 行 + MetadataReader 查询式操作例外 EXIF + stub plugin 模式 + 权限 Alpha advisory + panels + Vitest 测试模式)。`astro.config.mjs` sidebar 把原单条 Architecture 改为折叠组(Overview + 4 子项,放在 Getting Started 之后)。验证:`pnpm build` 成功 446 页(原 442 + 4 architecture),Pagefind 索引 446 HTML,4 个深度文路径正确生成 |
| 2026-07-05 | v4.2 | **W13.3 Guides 补充 4 篇**:新建 `apps/docs/src/content/docs/guides/` 目录,4 篇分步指南:① `embed-sdk.md` 嵌入 SDK guide(React 19 + Vite 主机应用嵌入 `<Workspace />` + 6 props 表 + COOP/COEP);② `write-first-plugin.md` 写第一个插件 guide(从零构建 `@lokvis/blur-image` 完整流程:capability 声明 → engine 操作 Blob→Blob `Record<string, any>` 签名 + AGENTS.md 禁双断言说明 → `definePlugin` + `createBlobCapabilityImpl` 装配 → loadPlugin → workflow 调用 + PluginContext 9 API 表 + stub 引擎处理 + 权限 + Vitest 测试);③ `custom-workspace.md` 自定义 Workspace guide(无 ui-react 仅 sdk + plugin-image 构建最小图像工作台 + `LokvisError` 错误体系 `fromLokvisError` 归一 + `DegradationRejectedError` instanceof 窄化 + 自定义 vs `<Workspace />` 选择表);④ `cli-automation.md` CLI 自动化 guide(`runCLI(['capabilities'])` + `validateWorkflow` Zod `SafeParseReturnType` shape + Node 限制说明 + 6 命令表)。`astro.config.mjs` sidebar 加 `Guides` 折叠组(Workflows 与 SDK 之间);`index.md` Next Steps 加 4 个 guides 链接。验证:`pnpm build` 成功 442 页(原 438 + 4 guides),Pagefind 索引 442 HTML,4 个 guides 路径正确生成 |
| 2026-07-05 | v4.1 | **W13.2 API Reference 自动生成(TypeDoc + starlight-typedoc)**:① `apps/docs/package.json` 加 devDeps typedoc ^0.28.19 + typedoc-plugin-markdown ^4.12.0 + starlight-typedoc ^0.23.0;② `apps/docs/typedoc.json` 配置 9 个核心包入口(schema/runtime/sdk/capability/plugin-sdk/plugin-image/engine-image/ui-react/ui-core)+ excludePrivate/Protected/Internal/Externals + sort=source-order;③ `tsconfig.typedoc.json` 根目录新建(paths 映射 9 包到 src + include 9 包源码 + exclude 测试);④ `apps/docs/astro.config.mjs` 集成 `starlightTypeDoc` 插件(sidebar collapsed + label 'API Reference');⑤ `apps/docs/src/content.config.ts` 修复——补 `schema: docsSchema()`(原未应用 Starlight schema,TypeDoc 生成页面无 `draft: false` 被 production build 过滤,438 页只构建 11 页);⑥ `.gitignore` 加 `apps/docs/src/content/docs/api/`(生成产物不入库);⑦ `apps/docs/package.json` clean 脚本加 `src/content/docs/api`。**验证**:427 个 API markdown 页自动生成,`pnpm build` 成功 438 页(原 11 + 427 API),Pagefind 索引 438 HTML。tsdoc 注释覆盖率良好(核心包已有完整 JSDoc + @example + @public),无需补充 |
| 2026-07-04 | v4.0 | **W13.1 npm 包 package.json 审计 + changeset 配置**:全量 18 个 `@lokvis/*` 包审计。① `.changeset/config.json` `baseBranch` `"main"`→`"dev"`;② 18 个 packages 补 `sideEffects`(ui-core/ui-react `["**/*.css"]`,其余 16 包 `false`);③ 3 个核心包版本统一 sdk/ui-react/ui-core `0.1.1-beta.0`→`0.2.0-beta.0`;④ cli 补顶级 `module`+`types`;⑤ apps 补 `license: MIT`+`sideEffects: false`。W13.5/W13.6 已在 W12.5/W12.6 完成。验证:typecheck 36/36、test 777/777、build 20/20 |
